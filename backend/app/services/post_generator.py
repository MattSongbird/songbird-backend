...
# Define the internal LangGraph state
class ToolState(BaseModel):
    input: str
    prompt: Optional[str] = None
    output: Optional[str] = None
    voice_style: Optional[str] = "neutral-professional"
    tool: str = "post-generator"
...
def build_prompt(state: ToolState) -> ToolState:
    try:
        prompt = f"Write a {state.voice_style} social media post about: {state.input}"
        state.prompt = prompt
        return state
...
# Node 3: Wrap in ToolOutput format
...
# Exposed entrypoint
def run_post_generator(req: dict) -> None:
    graph = StateGraph()
